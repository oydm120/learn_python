...
class PUBGAPI:
    ...
    def player_mode_stats(self, player_handle, game_mode='all', game_region='all'):
        ...
        """
        if game_mode not in ['solo', 'duo', 'squad', 'all']:
            raise APIException("game_mode must be one of: solo, duo, squad, all")
        if game_region not in ['as', 'na', 'agg', 'sea', 'eu', 'oc', 'sa', 'all']:
